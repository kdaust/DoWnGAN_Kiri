...
class WassersteinGAN:
    ...
    def _generator_train_iteration(self, coarse, fine, invariant):
        ...
        fake_means = torch.empty([coarse.shape[0],fine.shape[1],fine.shape[2],fine.shape[3]])
        c_result = torch.empty([coarse.shape[0]])
        for img in range(coarse.shape[0]):
            ...
            fake_means[img,...] = fake_mean
            c_result[img] = torch.mean(c_fake_stoch)
