...
highres_in = False
data_folder = "~/Masters/Data/processed_data/toydata/"
...
if(highres_in):
    ...
    coarse_train = torch.from_numpy(coarse_train).transpose(0, 1).to(config.device).float()
    fine_train = torch.from_numpy(fine_train).transpose(0, 1).to(config.device).float()
    coarse_test = torch.from_numpy(coarse_test).transpose(0, 1).to(config.device).float()
    fine_test = torch.from_numpy(fine_test).transpose(0, 1).to(config.device).float()
